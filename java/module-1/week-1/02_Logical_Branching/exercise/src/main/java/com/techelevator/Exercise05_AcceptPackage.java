...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        boolean overMaxDimension = (lengthInches > MAX_DIMENSION_INCHES) || (widthInches > MAX_DIMENSION_INCHES) || (heightInches > MAX_DIMENSION_INCHES);
        if (weightPounds <= MAX_WEIGHT_POUNDS && (lengthInches * widthInches * heightInches <= MAX_CUBIC_INCHES) && overMaxDimension && isSurchargePaid) {
            return true;
        } else if (weightPounds <= MAX_WEIGHT_POUNDS && (lengthInches * widthInches * heightInches <= MAX_CUBIC_INCHES) && overMaxDimension) {
            return false;
