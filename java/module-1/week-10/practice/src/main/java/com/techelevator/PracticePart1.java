...
public class PracticePart1 {
    ...
    */
    public String Q06_stringBetweenTags(String stringToSearch, String tag) {
        ...
        String tagAtTheBeginning = stringToSearch.substring(indexA + tag.length()); //takes out tag at the beginning
        if (tagAtTheBeginning.contains(tag)) {
            int indexB = tagAtTheBeginning.indexOf(tag);
            newString = tagAtTheBeginning.substring(0, indexB);
        } else {
            newString = "";
        }
        return newString;
    ...
    */
    public boolean Q07_isPalindrome(String word) {
        ...
        String reverseToUpper = reverseWord.toUpperCase();
        return wordToUpper.contains(reverseToUpper);
    }
